...
//const game = new GameView(initialState());
//game.onBack = () => changeView(greeting());

//game.onTripleAnswer = (e, level, state) => {
//  nextLevel(level.options[e.target.dataset.option].type === `paint`, state);
//};
//game.onRadioAnswer = (form, fields, level, state) => {
//  const checkedInputs = form.querySelectorAll(`input[type="radio"]:checked`);
//  if (checkedInputs.length === fields.length) {
//    let correctAnswersCount = 0;
//    checkedInputs.forEach((checkedInput, i) => {
...
//      }
//    });
//    nextLevel(correctAnswersCount === fields.length, state);
//  }
//};
